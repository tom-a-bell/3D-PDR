...
!!$!     Resize the collisional rate coefficient arrays to fit exactly the
!!$!     maximum number of temperatures required by the collision partners
!!$      COOLANT(N)%TEMPERATURE=RESHAPE(COOLANT(N)%TEMPERATURE,(/7,MAX_NTEMP/))
!!$      COOLANT(N)%C_COEFF=RESHAPE(COOLANT(N)%C_COEFF,(/7,NLEVEL,NLEVEL,MAX_NTEMP/))
