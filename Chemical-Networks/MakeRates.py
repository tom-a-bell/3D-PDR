...
# Check the conservation of elemental constituents in each reaction
def check_element_conservation(reactants, products, speciesList, elementList, speciesConstituents):
    failStatus = False
    nReactions = len(reactants)
    nElements = len(elementList)
    if useTtk:
    ...
    for n in range(nReactions):
        ...
        for i in range(nElements):
            ...
            if i == elementList.index('-'): continue
            # Produce an error message if an element is not conserved
            if productConstituents[i] != reactantConstituents[i]:
                failStatus = True
                if useTkinter:
        ...
        # Produce an error message if the net charge is not conserved
        if productConstituents[-1] != reactantConstituents[-1]:
            failStatus = True
            if useTkinter:
...
#                 return
        ...
        if speciesSizeLimit > 0:
            self.statusMessage('\nDiscarding species containing more than '+str(speciesSizeLimit)+' atoms (not counting H or D atoms)...')
...
#             return
        ...
        failed = check_element_conservation(reactants, products, speciesList, elementList, constituentList)
        if failed: return
        self.statusMessage('\n')
...
# Otherwise, use command line print statements to inform the user of progress
    ...
    # Check that all elemental constituents and the net charge are conserved in each reaction
    print '\nChecking element and charge conservation for each reaction...'
    status = check_element_conservation(reactants, products, speciesList, elementList, constituentList)
